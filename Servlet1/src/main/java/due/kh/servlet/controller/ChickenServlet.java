package due.kh.servlet.controller;
...
//@WebServlet("/") 슬래쉬 부터 시작.
@WebServlet("/chicken")
public class ChickenServlet extends HttpServlet{
	
	//HttpServletRequest : 요청자 정보, 데이터가 담긴 객체 
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//1.) 값이 하나인 경우 : String name req.getParameter("name")
		String chicken = req.getParameter("chicken"); // 치킨 얻어오기.
		String type = req.getParameter("type"); //뼈/순살 얻어오기.
		
		String ordererName = req.getParameter("ordererName"); // 주문자명
		String ordererAddress = req.getParameter("ordererAddress"); // 주소
		
		//2.) 값이 여러개인 경우
		// String[] name = req.getPrameterValues("name");
		// -> 제출된 값 중 name속성 값이 일치하는 것을 모두 모아
		// 	하나의 String[]로 반환 
		
		// 단, name속성 값이 일치하는게 없을 경우
		// "null" 반환.
		...
		case "양념"	 : 		 price += 11000;  break; //치킨 가격.
