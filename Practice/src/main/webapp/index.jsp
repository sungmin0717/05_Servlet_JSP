...
<body>
  ...
  <pre>
    ...
    - src/main/webapp
      ...
      - 그래서 파일 경로, 요청 주소 작성 시
        ...
        ex) 
        주소에 / : webapp의 index.jsp 요청

        forward 시 JSP 파일 경로 / : webapp 폴더 기준

        css,js파일 경로 : (== 요청주소) / : webapp 폴더 기준
        ...
        *src/main/webapp/WEB-INF 폴더
          ...
          - 이럴꺼면 배포하는 webapp폴더에 안넣으면 되는거 아닌가?
            -> 직접 해당 파일을 보여줄 순 없지만
              응답 화면 등의 웹 서비르를 제공하기 위해서는 
                필요하기 때문에 webapp 폴더에 존재해야한다.
  </pre>
  ...
  - 존재하는 경우
    -> "000 님은 X번째 인덱스에 존재 합니다."
        문자열을 request socpe 객체에 세팅.

        /WEB-INF/views/result.jsp 로 요청위임해 결과 출력.
        (forward)

  - 존재하지 않은 경우
    ->"000님은 존재하지 않습니다."
      문자열을 session scope에 세팅 후 
      "/" (메인 페이지 == index.jsp)로 재요청하여 출력(redirect)
