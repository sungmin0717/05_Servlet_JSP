...
<body>
  ...
  <pre>
    ...
    index.jsp가 변환된 index_jsp.class 파일로 요청이 매핑되며
    -> 클라이언트에게 index.jsp 코드가 그대로 출력됨.

    index.html index.jsp 차이.

    html : html,css,js 코드작성가능
      + 정적 페이지(미리 만들어진 형태 그대로)

    jsp : html,css,js,java,el,tstl 작성가능 
      +동적페이지(요청에 따라 응답 화면 변함 가능)
...
<h3>forward : 요청 위임</h3>
  ...
  - 클라이언트 요청을 받은 Servlet/JSP가
    직접 응답하는 것이 아닌,
      다른 Servlet/JSP에
      HttpServletRquest, HttpServletResponse 객체를 넘겨서 (위임)
      대신 응답하게 하는 것.
  ...
  <form action="/fr/forward">
    forword 입력 1 : <input type="text" name="forwardInput1">
    <br>
    forword 입력 2 : <input type="text" name="forwardInput2">
...
<h3>redirect()</h3>
  ...
  - redirect는 다시 요청하는것!!
    -> 기존 Client의 req,resp 객체가 사라지고
      새로운 req,resp객체가 생성된다!!!
      -> request scope 범위에 세팅된 값들이 모두 사라진다.

      
  - redirect는 응답 화면의 주소가
    처음 요청한 Servlet 주소가 아닌 
      다시 요청한 Servlet주소로 변경된다!!!
